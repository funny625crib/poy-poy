...
//! @brief モデルコンポーネントクラス
class ComponentPhysics final : public Component
...
public:
	...
	inline void SetOverlapCollision( u16 hit )
	{
		overlap_status_ = hit;
	}

	inline const u16 GetOverlapCollision() const
	{
	...
	inline const ComponentPhysics::CollisionType GetCollisionType() const
	{
		return type_;
	}

	inline void SetGravityFactor( float fact )
	{
		gravity_factor_ = fact;
	}

	inline const float GetGravityFactor() const
	{
	...
	inline void SetPhysicsReal( float pow )
	{
		physics_real_ = pow;
	}
	...
	inline const float GetPhysicsReal() const
	{
		return physics_real_;
	}
