...
void BettyStateIdleWalk::Init()
{
    __super::Init();
}

void BettyStateIdleWalk::Update()
{
    __super::Update();
    ...
    // 移動方向がランダムする
    static int betty_dir = 2;    //移動方向

    static int betty_wait_frame;    //移動方向を変わる前に待つ時間

    betty_wait_frame++;
    if(betty_wait_frame >= 180) {
        betty_dir        = GetRand(3);
        betty_wait_frame = 0;
    }
...
{
    move_speed_ = speed;
    return std::dynamic_pointer_cast<BettyStateIdleWalk>(shared_from_this());
}
BettyStateIdleWalkPtr BettyStateIdleWalk::SetRotateSpeed(const float speed)
{
    rot_speed_ = speed;
    return std::dynamic_pointer_cast<BettyStateIdleWalk>(shared_from_this());
}

const float BettyStateIdleWalk::GetMoveSpeed() const
{
    return move_speed_;
}

const float BettyStateIdleWalk::GetRotateSpeed() const
{
    return rot_speed_;
}

void BettyStateIdleWalk::GUI()
{
    __super::GUI();
...
CEREAL_REGISTER_TYPE(BettyStateIdleWalk)
CEREAL_REGISTER_POLYMORPHIC_RELATION(Component, BettyStateIdleWalk)
