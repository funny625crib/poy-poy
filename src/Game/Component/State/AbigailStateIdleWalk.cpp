...
void AbigailStateIdleWalk::Init()
{
    __super::Init();
}

void AbigailStateIdleWalk::Update()
{
    __super::Update();
    ...
    abigail_wait_frame++;
    if(abigail_wait_frame >= 180) {
        abigail_dir        = GetRand(3);
        abigail_wait_frame = 0;
    }
...
{
    move_speed_ = speed;
    return std::dynamic_pointer_cast<AbigailStateIdleWalk>(shared_from_this());
}
AbigailStateIdleWalkPtr AbigailStateIdleWalk::SetRotateSpeed(const float speed)
{
    rot_speed_ = speed;
    return std::dynamic_pointer_cast<AbigailStateIdleWalk>(shared_from_this());
}

const float AbigailStateIdleWalk::GetMoveSpeed() const
{
    return move_speed_;
}

const float AbigailStateIdleWalk::GetRotateSpeed() const
{
    return rot_speed_;
}

void AbigailStateIdleWalk::GUI()
{
    __super::GUI();
...
CEREAL_REGISTER_TYPE(AbigailStateIdleWalk)
CEREAL_REGISTER_POLYMORPHIC_RELATION(Component, AbigailStateIdleWalk)
